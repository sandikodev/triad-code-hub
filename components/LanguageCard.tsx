    ...
    <div 
      ...
      className={`relative group transition-all duration-500 p-6 rounded-2xl border-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 ${
        isSoon 
          ...
          : isActive 
            ? isPro 
              ? 'border-amber-500 bg-slate-800 shadow-lg shadow-amber-500/10 translate-y-[-4px] cursor-pointer'
    ...
    >
      <div className="absolute top-4 left-4 z-10 flex gap-2">
        {isSoon ? (
          ...
          <span className="px-2 py-0.5 rounded-md bg-amber-500 border border-amber-400 text-[8px] font-black text-slate-950 uppercase tracking-[0.2em] shadow-[0_0_10px_rgba(245,158,11,0.5)]">
            Architect Pro
          </span>
        ) : (
          <span className="px-2 py-0.5 rounded-md bg-slate-800 border border-white/10 text-[8px] font-black text-slate-400 uppercase tracking-[0.2em]">
      ...
      <div className={`absolute top-4 right-4 text-4xl opacity-50 transition-opacity ${!isSoon && 'group-hover:opacity-100'}`} aria-hidden="true">
        {isPro && !isActive ? '🔒' : info.icon}
      </div>
      
      <div className={`w-12 h-12 rounded-lg ${info.color} flex items-center justify-center mb-4 shadow-inner relative overflow-hidden mt-4`} aria-hidden="true">
      ...
      <p className={`text-sm font-medium mb-3 ${isSoon ? 'text-slate-600' : isPro ? 'text-amber-500/80' : info.accent}`}>{info.tagline}</p>
      ...
      {!isSoon ? (
        <div className={`flex items-center text-xs font-semibold ${isPro ? 'text-amber-500 group-hover:text-amber-400' : 'text-indigo-400 group-hover:text-indigo-300'}`} aria-hidden="true">
          {isPro ? 'UPGRADE ACCESS' : 'START LEARNING'}
